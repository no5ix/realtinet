...
class TcpLengthHeaderCodec{
public:
	typedef std::function<void(const muduo::net::TcpConnectionPtr&,
		const muduo::string& message,
		muduo::Timestamp)> TcpStringMessageCallback;

	explicit TcpLengthHeaderCodec(const TcpStringMessageCallback& cb)
	...
	{
		//LOG_INFO << "into TcpLengthHeaderCodec::onMessage";
		...
		while (buf->readableBytes() >= kHeaderLen) // kHeaderLen == 4
		{
			...
			else if (buf->readableBytes() >= len + kHeaderLen)
			{
				...
				//LOG_INFO << "into TcpLengthHeaderCodec::onMessage messageCallback_(conn, message, receiveTime);";
				buf->retrieve(len);
			}
			else
			{
	...
	//explicit TcpLengthHeaderCodec(const UdpStringMessageCallback& cb)
